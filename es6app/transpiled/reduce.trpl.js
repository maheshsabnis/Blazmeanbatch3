...
var sum = 0; // Using forEach

values.forEach(function (n, i) {
  sum += n;
});
console.log("Sum = ".concat(sum)); // Using Array.reduce();
...
var resSum = values.reduce(function (prevSum, currentVal, index, val) {
  console.log("prevSum = ".concat(prevSum, " currentVal = ").concat(currentVal, " index = ").concat(index, " vall = ").concat(val));
  return prevSum + currentVal;
});
console.log("Using array.reduce() sum = ".concat(resSum)); // Case 2: Finding out a frequency of a record in array

var records = ['A', 'B', 'C', 'D', 'E', 'D', 'C', 'A', 'D', 'B', 'A', 'E', 'F', 'D', 'A', 'C', 'D', 'B', 'A', 'E', 'C']; // the prevRec will be the final state of the array that stores the result with frequency 

var frequency = records.reduce(function (prevRec, curRec) {
  console.log("preRec = ".concat(JSON.stringify(prevRec), " and curRec = ").concat(curRec)); // 'in' operator for if statement is same as iteration till end-of-records in collection 
  ...
  if (curRec in prevRec) {
    ...
    console.log("In if True ".concat(JSON.stringify(prevRec)));
  } else {
...
console.log("Frequency Per Record in Array is = ".concat(JSON.stringify(frequency), " ")); // Case 3: Create a group of records from a complex array based on group property
